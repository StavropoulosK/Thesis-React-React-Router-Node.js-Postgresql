        ...
        <>
            ...
            <nav className="userNav">
                ...
                {options.map((option,index)=>{
                    return(
                    <NavLink to={option.linkTo} key={index} className={({ isActive, isPending }) => isActive ? "active" : ""}>

                        {option.svg}
                        { showNavBar&& <span>{t(option.text)}</span>}
                    </NavLink>)
                ...
                <button onClick={()=>setShowNavBar(prev=>!prev)} className="navElement" >
                    ...
                    {showNavBar && <span>{t("Collapse")}</span>}
